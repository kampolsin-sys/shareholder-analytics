...
with tab2:
    st.header("อัปโหลดไฟล์ข้อมูลดิบ (Excel)")
                ...
                with st.spinner("กำลังคำนวณ..."):
                    ...
                    if comp_df.empty:
                        ...
                        with table_tab:
                            ...
                            """
                            if hasattr(st, 'html'):
                                st.html(custom_css + f'<div class="table-container">{html_table}</div>')
                            else:
                            ...
                            with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                                # Export raw numeric data without pandas index
                                comp_df.to_excel(writer, sheet_name='Comparison', index=False)
                                
                                ws = writer.sheets['Comparison']
                                
                                # Insert 2 rows for the title
                                ws.insert_rows(1, 2)
                                ...
                                # Because of index=False and MultiIndex, row 3 has top headers, row 4 has bottom headers
                                # 'ลำดับ' and 'รายชื่อผู้ถือหุ้น' have empty top headers, so we merge rows 3 and 4
                                ws.merge_cells('A3:A4')
                                ws.merge_cells('B3:B4')
                                ws['A3'] = 'ลำดับ'
                                ws['B3'] = 'รายชื่อผู้ถือหุ้น'
